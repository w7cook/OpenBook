...
public class SkinFunctionalTest extends FunctionalTest {
  ...
  public void testRedirect() {
    Response response = GET("/changeSkin");
    assertStatus(Http.StatusCode.FOUND, response);
    assertHeaderEquals("Location", "/changeSkin", response);
    
    response = GET("/editMySkin");
    assertStatus(Http.StatusCode.FOUND, response);
    assertHeaderEquals("Location", "/editMySkin", response);
    
    response = GET("/sampleSkin");
    assertStatus(Http.StatusCode.FOUND, response);
    assertHeaderEquals("Location", "/sampleSkin", response);
    
    response = GET("/changeBGImage");
    assertStatus(Http.StatusCode.FOUND, response);
    assertHeaderEquals("Location", "/changeBGImage", response);
  }
}
